...
const INITIAL_GREETING =
	'Hello! I am Vista, your AI health assistant. How can I help you today?'
...
export async function POST(req: NextRequest) {
	...
	try {
		...
		const recentRequests = clientRequests.filter(
			(time: number) => now - time < RATE_LIMIT.windowMs
		)
		...
		const bmi =
			userData.height && weight !== 'Unknown'
				? (Number(weight) / Math.pow(Number(userData.height) / 100, 2)).toFixed(
						2
				  )
				: 'Unknown'
...
Remember, individual needs may vary. It's always best to consult with a healthcare professional or registered dietitian for personalized advice.
    ...
    Waist Circumference: ${
			waistCircumference !== 'Unknown' ? `${waistCircumference} cm` : 'Unknown'
		}
