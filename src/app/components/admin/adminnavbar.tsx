	...
	const navItems: NavItem[] = [
...

		{
			...
			submenu: [
				...
				{
					href: '/admin/coach-history',
					label: 'Coaches History',
					icon: FaUserTie
				}
			]
		}
						...
						<div
							...
							{item.submenu.map((subItem, subIndex) => (
								<Link
									...
									className={`${
										isMobile ? 'block' : ''
									} px-4 py-2 flex items-center text-sm text-gray-700 hover:bg-green-300 hover:text-white ${
				...
				{isMenuOpen && (
					<motion.div
						...
						<div className='px-2 pt-2 pb-3 space-y-1 sm:px-3'>
							{navItems.map((item, index) => (
								<div key={index}>{renderNavItem(item, true)}</div>
							))}
						</div>
					</motion.div>
				)}
