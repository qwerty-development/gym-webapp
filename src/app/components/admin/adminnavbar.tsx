...
export default function AdminNavbarComponent() {
  ...
  const navItems: NavItem[] = [
    ...
    },
    {
      ...
      submenu: [
        { href: '/admin/view-reservations', label: 'View All', icon: FaClipboardList },
        { href: '/admin/calendar-view', label: 'Calendar', icon: FaCalendarAlt },
      ]
    },
    {
    ...
    if (item.submenu) {
      return (
        <div className="relative">
          ...
          {openSubMenu[item.label] && (
            <div className={`${isMobile ? 'mt-2 ml-6 space-y-1' : 'absolute z-10 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5'}`}>
              {item.submenu.map((subItem, subIndex) => (
                <Link
                  key={subIndex}
                  href={subItem.href!}
                  className={`${isMobile ? 'block' : ''} px-4 py-2 flex items-center text-sm text-gray-700 hover:bg-green-300 hover:text-white ${
                    currentPage === subItem.href ? 'bg-green-500 text-white' : ''
                  }`}
